	...
	<form action="tproductController/${msg}.do" name="Form" id="Form" method="post">
		...
		<table id="table_report" class="table table-striped table-bordered table-hover">
			...
			<tr>
				...
				<td><textarea  cols="115"  rows="5"   style="height: 100%;width: 100%;"  name="F_PRODUCTDESC" id="F_PRODUCTDESC"   placeholder="这里输入商品描述" title="商品描述">${pd.F_PRODUCTDESC}</textarea></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
				   <div class="radio" style="float:left">
								<label >
									<input type="radio" class="ace" name="F_ISSPECIAL_label" <c:if test='${pd.F_ISSPECIAL==0}'>checked="checked"</c:if>>
				     ...
				     <div class="radio" style="margin-left:10px;">
				               <label>
									<input type="radio" class="ace" name="F_ISSPECIAL_label" <c:if test='${pd.F_ISSPECIAL==1}'>checked="checked"</c:if>>
									<span class="lbl" style="margin-left: 20px;"> 否</span>
								</label>  
				    </div> 
				<input type="hidden" name="F_ISSPECIAL" id="F_ISSPECIAL" value="<c:if test='${pd.F_ISSPECIAL==0}'>是</c:if><c:if test='${pd.F_ISSPECIAL==1}'>否</c:if>" maxlength="32" placeholder="这里输入是否特卖" title="是否特卖"/>
			</tr>
