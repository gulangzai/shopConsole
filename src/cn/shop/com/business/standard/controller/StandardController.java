package cn.shop.com.business.standard.controller;
...
public class StandardController extends BaseController {
	...
	String menuUrl = "standard/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, "新增Standard");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("STANDARD_ID", this.get32UUID());	//主键
	...
	public ModelAndView list(Page page){
		...
		try{
			pd = this.getPageData();
			page.setPd(pd);
			List<PageData>	varList = standardService.list(page);	//列出Standard列表
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增Standard页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("information/standard/standard_add");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改Standard页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd = standardService.findById(pd);	//根据ID读取
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出Standard到excel");
