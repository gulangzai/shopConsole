package cn.shop.com.business.tbshouchang.controller;
...
import cn.shop.com.business.tbshouchang.service.TbShouchangService;
import net.sf.json.JSONArray;

import com.fh.util.Jurisdiction; 

/** 
 * 类名称：tbShouchangController
 * 创建人：李益  收藏
 * 创建时间：2016-06-05
 */
@Controller
@RequestMapping(value="/tbShouchangController")
public class TbShouchangController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		int productId = tbShouchangService.save(pd);
	...
	public ModelAndView list(Page page){
		...
		try{
			pd = this.getPageData();
			page.setPd(pd);
			List<PageData>	varList = tbShouchangService.list(page);	//列出TProductController列表
	...
	public ModelAndView goEdit(){
		...
		pd = this.getPageData();
		try {
			pd = tbShouchangService.findById(pd);	//根据ID读取
	...
	public ModelAndView exportExcel(){
		...
		try{
			...
			List<PageData> varOList = tbShouchangService.listAll(pd);
	...
	@RequestMapping(value="/mobileListByUserId")
	@ResponseBody
	public JSONPObject  mobileListByUserId(Page page,Model model,HttpServletRequest req,HttpServletResponse response){
		...
		try {  
			varList = tbShouchangService.findByByUserId(page); 
	...
	@RequestMapping(value="/mobileListAdd")
	@ResponseBody
	public JSONPObject  mobileListAdd(Page page,Model model,HttpServletRequest req,HttpServletResponse response){
		response.setHeader("Access-Control-Allow-Origin", "*");
	    String callbackFunName =req.getParameter("callbackparam");//得到js函数名称  
		...
		List<PageData> varList = null;  
		try {
			if("add".equals((String)pd.get("LABEL"))){
				varList = tbShouchangService.mobileListAdd(page); 
				
			}else if("delete".equals((String)pd.get("LABEL"))){
				varList = tbShouchangService.mobileListDelete(page); 
			}
			JSONArray classes = JSONArray.fromObject(varList); 
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	public JSONPObject  mobileProductById(Page page,Model model,HttpServletRequest req,HttpServletResponse response){
		...
		try {  
			tbProduct = tbShouchangService.findById(pd);  
